...
<Callout type="info" title="What AO installs">
	The recommended install is the **desktop app** from GitHub Releases. It bundles the daemon, dashboard, and plugins, and
	updates itself. The `@aoagents/ao` npm package is the legacy on-ramp: it ships the `ao` CLI, whose `ao start` fetches and
	opens the same desktop app. The npm package is frozen and no longer updated.
</Callout>
...
<Callout type="warn" title="npm is the legacy path, no longer recommended">
	The `@aoagents/ao` npm package is frozen and no longer receives updates. It ships the `ao` CLI for existing users;
	`ao start` fetches and opens the same desktop build above. Use it only if you already run AO from the CLI:
</Callout>
