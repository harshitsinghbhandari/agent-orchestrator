...
export function GET() {
  ...
  const lines: string[] = [
    ...
    "## Capabilities",
    "",
    `Via the MCP server an authenticated agent can, on behalf of an ${COMPANY.NAME} user:`,
    "",
    ...
    `- Alternatively, a user-issued ${COMPANY.NAME} API key can be sent as a Bearer token.`,
    "",
    "## Setup one-liners",
    "",
    "```bash",
    `claude mcp add ao-agents --transport http ${MCP_SERVER_URL}`,
    `codex mcp add ao-agents --url ${MCP_SERVER_URL}`,
    `gemini mcp add --transport http ao-agents ${MCP_SERVER_URL}`,
    "```",
    "",
    "## Learn more",
    "",
